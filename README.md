...
| `SHIM_FORCE_THINKING` | *(auto)* | `enabled`/`disabled` = force `thinking:{type}` on every body; empty = auto (inject `enabled` only when `SHIM_TARGET` matches `z.ai`/`bigmodel`); `off` = never inject. **All production launchers set `enabled`.** |
