...
client ──▶ http://127.0.0.1:8797/v1 ──▶ https://api.z.ai/api/coding/paas/v4   (GLM-5.3-Flash via Z.ai official, effort=low)
...
Z.ai GLM, DeepSeek and Kimi K3 (via OpenCode Go) reasoning models require a non-standard field, `reasoning_content`, on **every assistant message that carries `tool_calls`** in the conversation history. Standard OpenAI SDK / OpenAI-compatible clients drop that field, so the **turn after any tool call fails with `400`**:
...
- **Resilience** — retries transient upstream errors, keeps SSE streams alive with keepalive comments, and survives crashes. Every launcher runs under an auto-restart wrapper, and `start-shim-hidden.vbs` starts all instances at logon.
...
- **Thinking injection** — GLM/DeepSeek/Kimi reasoning models only emit `reasoning_content` when the request body carries `{"thinking": {"type": "enabled"}}`. Every production instance runs with `SHIM_FORCE_THINKING=enabled`, so thinking is on everywhere without client-side changes. Controlled by `SHIM_FORCE_THINKING`.

## Requirements

- Node.js ≥ 20 (tested on Node 22)
- API keys for the upstreams you use (Z.ai / DeepSeek / OpenCode Go)
...
- `start-shim-opencode.cmd` — OpenCode Go (DeepSeek) relay on port `8792` (`start-shim-opencode.ps1`).
...
- `start-shim-zai-low.cmd` — Z.ai official (GLM-5.3-Flash, **reasoning_effort=low**) relay on port `8797` (`start-shim-zai-low.ps1`).
...
| OpenCode Go (DeepSeek) | `start-shim-opencode.cmd` | `8792` | `https://opencode.ai/zen/go/v1` |
...
| Z.ai official (GLM-5.3-Flash, effort low) | `start-shim-zai-low.cmd` | `8797` | `https://api.z.ai/api/coding/paas/v4` |
...
- OpenCode Go (DeepSeek) → `http://127.0.0.1:8792/v1`
- OpenCode Go (GLM-5.3-Flash) → `http://127.0.0.1:8793/v1`
- OpenCode Go (Kimi K3) → `http://127.0.0.1:8794/v1`
- Z.ai official (GLM-5.3-Flash, effort low) → `http://127.0.0.1:8797/v1`
...
| `SHIM_TARGET` | `https://api.moonshot.ai/v1` | Upstream API base URL (every production launcher overrides this) |
...
| `SHIM_REASONING_STORE_MAX` | `5000` | Max reasoning entries kept in memory (large default: eviction would silently break the prefix cache) |
...
| `SHIM_FORCE_THINKING` | *(auto)* | `enabled`/`disabled`/`off` = force `thinking:{type}` on every body (`off` writes `disabled` — honoured by DeepSeek, **ignored by GLM-5.3+**); empty = auto (inject `enabled` only when `SHIM_TARGET` matches `z.ai`/`bigmodel`). **All production launchers set `enabled`.** |
| `SHIM_REASONING_EFFORT` | *(empty)* | If set (e.g. `low`), forces `reasoning_effort` on every body. Z.ai docs: GLM-5.3/FLASH support `max`/`high`/`low` only; `low` = mild inference (cheapest official thought mode). **`start-shim-zai-low.cmd` sets `low`.** |
...
All output goes to `moonshot-shim.log` in the repo root (or `SHIM_LOG`), rotated when it exceeds 5 MB. Each instance appends to the same file; model ids in the lines tell instances apart. Check it first when diagnosing issues.
...
Boots a mock echo server, starts a fresh shim against it, sends a multi-turn tool-call conversation, and asserts `reasoning_content` is injected.
